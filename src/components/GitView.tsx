...
export default function GitView() {
  const {
    stagedChanges, unstagedChanges, gitCommits,
    stageFile, unstageFile, commitChanges, pushChanges, pullChanges, generateCommitMessage,
  } = useIDEStore();
  ...
  const handleCommit = async () => {
    ...
    setActionStatus("Committed successfully");
  ...
  const handlePush = async () => {
    ...
    await pushChanges();
    setActionStatus("Pushed to remote");
    setTimeout(() => setActionStatus(null), 3000);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-bg-panel/50">
      <div className="p-4 border-b border-white/5 bg-white/2 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={handlePull} className="p-1.5 hover:bg-white/5 rounded text-text-dim hover:text-white transition-colors" title="Pull">
              <ArrowDown size={14} />
            </button>
            <button onClick={handlePush} className="p-1.5 hover:bg-white/5 rounded text-text-dim hover:text-white transition-colors" title="Push">
              <ArrowUp size={14} />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex justify-between items-center">
            <label className="text-[10px] font-bold text-text-dim uppercase tracking-widest">Commit Message</label>
            <button
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {stagedChanges.length > 0 && (
          <div className="mb-4">
            <div className="p-4 flex items-center justify-between bg-white/2 border-y border-white/5">
              <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">Staged</span>
            ...
            <div className="divide-y divide-white/2">
              {stagedChanges.map((file, i) => (
                <div key={i} className="px-4 py-2 hover:bg-white/5 flex items-center justify-between group transition-colors">
                  ...
                  <button onClick={() => unstageFile(file)} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-red-500/20 rounded transition-all text-text-dim hover:text-red-400">
                    <Minus size={12} />
                  </button>
                </div>
              ))}
        ...
        <div className="p-4 flex items-center justify-between bg-white/2 border-y border-white/5">
          <span className="text-[10px] font-bold text-amber-400 uppercase tracking-widest">Changes</span>
        ...
        {unstagedChanges.length === 0 && stagedChanges.length === 0 && (
          <div className="p-8 text-center text-text-dim text-xs italic opacity-50">No changes detected</div>
        )}
        ...
        <div className="divide-y divide-white/2">
          {unstagedChanges.map((file, i) => (
            <div key={i} className="px-4 py-2 hover:bg-white/5 flex items-center justify-between group transition-colors">
              ...
              <button onClick={() => stageFile(file)} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-brand/20 rounded transition-all text-text-dim hover:text-brand">
                <Plus size={12} />
              </button>
            </div>
          ))}
        ...
        {gitCommits.length > 0 && (
          <div className="mt-6 border-t border-white/5">
            <div className="p-4 bg-white/2 flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-text-dim uppercase tracking-widest">History</span>
            </div>
            <div className="divide-y divide-white/2">
      ...
      <div className="p-4 border-t border-white/5 bg-white/2">
        <div className="flex items-center justify-between text-text-dim">
          <span className="text-[10px] uppercase font-bold tracking-tighter opacity-50">CodeForge AI v2.0</span>
